...
public class EnemySpawner : MonoBehaviour
{
    ...
    public void Spawn(float chanceOfSpawn, int minNumSpawns, int maxNumSpawns)
    {
        ...
        if (Random.Range(0f, 1f) < chanceOfSpawn)
        {
            ...
            for (int i = 0; i < numSpawns; i++)
            {
                ...
                Vector3 direction = spawnPoint.position + (Random.insideUnitCircle * spawnRadius).AsVector3();
                Quaternion rotation = Quaternion.Euler(0, 0, Random.Range(0f, 360f));

                Enemy enemy = Instantiate(prefab, direction, rotation, spawnPoint)
                    .GetComponent<Enemy>();
